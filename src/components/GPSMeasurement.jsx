...
const GPSMeasurement = () => {
  ...
  useEffect(() => {
      if (activeMode !== 'default' && !tempMeasureData) {
          setMeasurePoints([]);
          setMarkerPoints([]);
      }
      if (activeMode !== 'default') setPreviewData(null);
  }, [activeMode, tempMeasureData]);

  useEffect(() => {
      if (previewData && map) {
          const timer = setTimeout(() => handleFitBounds(), 100);
  ...
  const [poiData, setPoiData] = useState({ title: '', description: '', group: { name: 'Without group', color: 'transparent' }, date: new Date().toLocaleDateString(), address: '' });
  ...
  const handleFitBounds = () => {
    let pointsToFit = [];
    if (previewData) pointsToFit = previewData.points;
    else if (activeMode === 'marker') pointsToFit = markerPoints;
    else if (activeMode === 'field' || activeMode === 'distance') pointsToFit = measurePoints;

    if (pointsToFit && pointsToFit.length > 0 && map && map._container) {
       try {
         const group = new L.featureGroup(pointsToFit.map(p => L.marker([p.lat, p.lng])));
         map.flyToBounds(group.getBounds().pad(0.2), { duration: 1 });
  ...
  const handleFinalSave = (finalPoiData) => { 
      if (tempMeasureData && tempMeasureData.id) { setSavedFiles(prev => prev.filter(f => f.id !== tempMeasureData.id)); }
      ...
      setShowSaveScreen(false); setMarkerPoints([]); setMeasurePoints([]); setTempMeasureData(null); setActiveMode('default'); setOverlayImage(null); alert("Saved successfully!"); 
  };
  
  const handleCreateGroup = (newGroup) => { setGroups([...groups, newGroup]); };
  const handleDeleteFile = (id) => { if (window.confirm("Delete this record?")) { setSavedFiles(prev => prev.filter(f => f.id !== id)); } };

  const handleEditFile = (file) => {
      setPoiData(file.details);
      setTempMeasureData({ ...file.geometry, id: file.id }); 
      if (file.type === 'field' || file.type === 'distance') setMeasurePoints(file.geometry.points);
      else if (file.type === 'marker') setMarkerPoints(file.geometry.points);
      setActiveMode(file.type);
      setShowFileManager(false);
      setMeasureMethod('manual');
  };

  const handlePreviewFile = (file) => {
      if (file.geometry && file.geometry.points && (file.type === 'field' || file.type === 'distance' || file.type === 'marker')) {
      ...
      {isSearchOpen && (
          <div style={styles.searchResultsList}>
            {searchQuery.length === 0 && searchHistory.length > 0 && (
                <> <div style={{padding: '10px 20px', fontSize: '12px', color: '#999', fontWeight: 'bold'}}>RECENT SEARCHES</div> {searchHistory.map((item, index) => ( <div key={`hist-${index}`} style={styles.searchResultItem} onClick={() => { if(map) map.flyTo([item.coords[1], item.coords[0]], 18); setIsSearchOpen(false); }}> <div style={{display:'flex', alignItems:'center'}}> <FiClock size={16} color="#999" style={{marginRight: 10}}/> <div><div style={{fontWeight: 'bold', color: '#000'}}>{item.name}</div><div style={{fontSize:'12px', color:'#666'}}>{item.city}</div></div> </div> </div> ))} </>
            )}
            {searchResults.map((feature, index) => ( <div key={index} style={styles.searchResultItem} onClick={() => { if (map) map.flyTo([feature.geometry.coordinates[1], feature.geometry.coordinates[0]], 18); addToHistory(feature); setSearchResults([]); setIsSearchOpen(false); }}> <div style={{fontWeight: 'bold', color: '#000'}}>{feature.properties.name}</div> <div style={{fontSize:'12px', color: '#666'}}>{[feature.properties.city, feature.properties.state, feature.properties.country].filter(Boolean).join(', ')}</div> </div> ))}
          </div>
      )}
