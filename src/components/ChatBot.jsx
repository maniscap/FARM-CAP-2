...
import { fetchChatCompletion, generateImage } from '../utils/AiBrain';
...
function ChatBot() {
  ...
  const MODEL_QUEUE = useMemo(() => [
    ...
    { provider: 'openrouter', id: 'deepseek/deepseek-v4-flash:free', vision: false, desc: "OR DeepSeek V4 Flash" },
    { provider: 'openrouter', id: 'deepseek/deepseek-r1:free', vision: false, desc: "OR DeepSeek R1" },
    { provider: 'openrouter', id: 'qwen/qwen-3-coder-32b:free', vision: false, desc: "OR Qwen 3 Coder" },
    { provider: 'openrouter', id: 'meta-llama/llama-4-70b-instruct:free', vision: false, desc: "OR Llama 4 70B" },
    { provider: 'openrouter', id: 'google/gemini-3.5-pro:free', vision: true, desc: "OR Gemini 3.5 Pro" },
    { provider: 'openrouter', id: 'google/gemini-3.5-flash:free', vision: true, desc: "OR Gemini 3.5 Flash" },
    { provider: 'openrouter', id: 'meta-llama/llama-3.3-70b-instruct:free', vision: false, desc: "OR Llama 3.3 70B" },
    { provider: 'openrouter', id: 'nvidia/nemotron-3-nano:free', vision: false, desc: "OR Nemotron 3 Nano" },
    ...
    // --- GROQ FAST TIER ---
    { provider: 'groq', id: 'gpt-oss-120b', vision: false, desc: "GPT-OSS 120B" },
    { provider: 'groq', id: 'gpt-oss-20b', vision: false, desc: "GPT-OSS 20B" },
    { provider: 'groq', id: 'llama-4-maverick', vision: true, desc: "Llama 4 Maverick Vision" },
    ...
    { provider: 'hf', id: 'meta-llama/Meta-Llama-4-8B-Instruct', vision: false, desc: "HF Llama 4 8B" },
    { provider: 'hf', id: 'meta-llama/Meta-Llama-3.1-8B-Instruct', vision: false, desc: "HF Llama 3.1 8B" },
    { provider: 'hf', id: 'Qwen/Qwen3-7B-Instruct', vision: false, desc: "HF Qwen 3 7B" },
    { provider: 'hf', id: 'google/gemma-3-9b-it', vision: false, desc: "HF Gemma 3 9B" },
    { provider: 'hf', id: 'microsoft/Phi-4-mini-instruct', vision: false, desc: "HF Phi-4 Mini" },
  ...
  const executeAILoop = async (text, rBase64, dUrl, mType) => {
    ...
    if (text.toLowerCase().startsWith("generate an image") || text.toLowerCase().startsWith("draw") || text.toLowerCase().startsWith("create an image")) {
        try {
            // Updated to use local AiBrain utility
            ...
            addMessage({ text: `⚠️ Image generation failed: ${e.message}`, sender: "bot", timestamp: new Date().toLocaleTimeString() });
            setIsLoading(false); setIsTyping(false);
            return;
        }
    }
    ...
    const fetchModel = async (model) => {
      try {
        const responseText = await fetchChatCompletion({
          model,
          text,
          rBase64,
          mType,
